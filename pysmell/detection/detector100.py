...
import astChecker
import customast
from parameter import subject_dir, repositories
...
LongParameterList.writerow(
    ['subject', 'file', 'link', 'lineno', 'PAR', 'experience-based', 'statistics-based', 'tuning machine', 'smell'])
LongMethod = csv.writer(file('result100/LongMethod.csv', 'wb'))
LongMethod.writerow(
    ['subject', 'file', 'link', 'lineno', 'MLOC', 'experience-based', 'statistics-based', 'tuning machine', 'smell'])
LongScopeChaining = csv.writer(file('result100/LongScopeChaining.csv', 'wb'))
LongScopeChaining.writerow(
    ['subject', 'file', 'link', 'lineno', 'DOC', 'experience-based', 'statistics-based', 'tuning machine', 'smell'])
LongBaseClassList = csv.writer(file('result100/LongBaseClassList.csv', 'wb'))
LongBaseClassList.writerow(
    ['subject', 'file', 'link', 'lineno', 'NBC', 'experience-based', 'statistics-based', 'tuning machine', 'smell'])
LargeClass = csv.writer(file('result100/LargeClass.csv', 'wb'))
LargeClass.writerow(
    ['subject', 'file', 'link', 'lineno', 'CLOC', 'experience-based', 'statistics-based', 'tuning machine', 'smell'])
LongLambdaFunction = csv.writer(file('result100/LongLambdaFunction.csv', 'wb'))
LongLambdaFunction.writerow(
    ['subject', 'file', 'link', 'lineno', 'NOC', 'PAR', 'NOO', 'experience-based', 'statistics-based',
     'tuning machine', 'smell' ])
LongTernaryConditionalExpression = csv.writer(file('result100/LongTernaryConditionalExpression.csv', 'wb'))
LongTernaryConditionalExpression.writerow(
    ['subject', 'file', 'link', 'lineno', 'NOC', 'NOL', 'experience-based', 'statistics-based', 'tuning machine', 'smell'])
ComplexContainerComprehension = csv.writer(file('result100/ComplexContainerComprehension.csv', 'wb'))
ComplexContainerComprehension.writerow(
    ['subject', 'file', 'link', 'lineno', 'NOC', 'NOFF', 'NOO', 'experience-based', 'statistics-based',
     'tuning machine', 'smell' ])
LongMessageChain = csv.writer(file('result100/LongMessageChain.csv', 'wb'))
LongMessageChain.writerow(
    ['subject', 'file', 'link', 'lineno', 'LMC', 'experience-based', 'statistics-based', 'tuning machine', 'smell'])
MultiplyNestedContainer = csv.writer(file('result100/MultiplyNestedContainer.csv', 'wb'))
MultiplyNestedContainer.writerow(
    ['subject', 'file', 'link', 'lineno', 'LEC', 'DNC', 'NCT', 'experience-based', 'statistics-based',
     'tuning machine', 'smell'])

for subjectName in subjects:
    sourcedir = subject_dir + '/' + subjectName
    counts = {'LongParameterList': [0, 0, 0, 0], 'LongMethod': [0, 0, 0, 0], 'LongScopeChaining': [0, 0, 0, 0],
              'LongBaseClassList': [0, 0, 0, 0], 'LargeClass': [0, 0, 0, 0], 'LongMessageChain': [0, 0, 0, 0],
              'LongLambdaFunction': [0, 0, 0, 0], 'LongTernaryConditionalExpression': [0, 0, 0, 0],
              'ComplexContainerComprehension': [0, 0, 0, 0], 'MultiplyNestedContainer': [0, 0, 0, 0]}
    if not os.path.isdir('result100/%s' % subjectName):
        os.mkdir('result100/%s' % subjectName)
    mylog = open('result100/%s/%s.csv' % (subjectName, subjectName),mode='wb+')
        ...
        for item in myast.result:
            if item[0] == 1:
                ...
                if flag0 or flag1 or flag2:
                    smell = True
                    counts['LongParameterList'][3] += 1
                else:
                    smell = False

                LongParameterList.writerow([subjectName,
                                            item[1],
                                            item[1].replace(
                                                '/Users/joaocorreia/PycharmProjects/Pysmell/pysmell/subjects/' + subjectName,
                                                repositories[subjectName].replace('.git','/blame/master')) + '#L' + str(item[2]),
                                            item[2],
                                            item[3],
                                            flag0,
                                            flag1,
                ...
                if flag0 or flag1 or flag2:
                    smell = True
                    counts['LongMethod'][3] += 1
                else:
                    smell = False

                LongMethod.writerow([subjectName,
                                     item[1],
                                     item[1].replace(
                                         '/Users/joaocorreia/PycharmProjects/Pysmell/pysmell/subjects/' + subjectName,
                                         repositories[subjectName].replace('.git', '/blame/master')) + '#L' + str(
                                         item[2]),
                                     item[2],
                                     item[3],
                                     flag0,
                ...
                if flag0 or flag1 or flag2:
                    smell = True
                    counts['LongScopeChaining'][3] += 1
                else:
                    smell = False
                LongScopeChaining.writerow([subjectName,
                                            item[1],
                                            item[1].replace(
                                                '/Users/joaocorreia/PycharmProjects/Pysmell/pysmell/subjects/' + subjectName,
                                                repositories[subjectName].replace('.git',
                                                                                  '/blame/master')) + '#L' + str(
                                                item[2]),
                                            item[2],
                                            item[3],
                ...
                if flag0 or flag1 or flag2:
                    smell = True
                    counts['LongBaseClassList'][3] += 1
                else:
                    smell = False

                LongBaseClassList.writerow([subjectName,
                                            item[1],
                                            item[1].replace(
                                                '/Users/joaocorreia/PycharmProjects/Pysmell/pysmell/subjects/' + subjectName,
                                                repositories[subjectName].replace('.git',
                ...
                if flag0 or flag1 or flag2:
                    smell = True
                    counts['LargeClass'][3] += 1
                else:
                    smell = False
                LargeClass.writerow([subjectName,
                                     item[1],
                                     item[1].replace(
                                         '/Users/joaocorreia/PycharmProjects/Pysmell/pysmell/subjects/' + subjectName,
                                         repositories[subjectName].replace('.git', '/blame/master')) + '#L' + str(
                ...
                if len(item) == 4:
                    ...
                    if flag0 or flag1 or flag2:
                        smell = True
                        counts['MultiplyNestedContainer'][3] += 1
                    else:
                        smell = False

                    MultiplyNestedContainer.writerow([subjectName,
                                                      item[1],
                                                      item[1].replace(
                                                          '/Users/joaocorreia/PycharmProjects/Pysmell/pysmell/subjects/' + subjectName,
                                                          repositories[subjectName].replace('.git',
                ...
                if flag0 or flag1 or flag2:
                    smell = True
                    counts['LongLambdaFunction'][3] += 1
                else:
                    smell = False

                LongLambdaFunction.writerow([subjectName,
                                             item[1],
                                             item[1].replace(
                                                 '/Users/joaocorreia/PycharmProjects/Pysmell/pysmell/subjects/' + subjectName,
                                                 repositories[subjectName].replace('.git',
                ...
                if flag0 or flag1 or flag2:
                    smell = True
                    counts['LongTernaryConditionalExpression'][3] += 1
                else:
                    smell = False

                LongTernaryConditionalExpression.writerow([subjectName,
                                                           item[1],
                                                           item[1].replace(
                                                               '/Users/joaocorreia/PycharmProjects/Pysmell/pysmell/subjects/' + subjectName,
                                                               repositories[subjectName].replace('.git',
                ...
                if flag0 or flag1 or flag2:
                    smell = True
                    counts['ComplexContainerComprehension'][3] += 1
                else:
                    smell = False
                ComplexContainerComprehension.writerow([subjectName,
                                                        item[1],
                                                        item[1].replace(
                                                            '/Users/joaocorreia/PycharmProjects/Pysmell/pysmell/subjects/' + subjectName,
                                                            repositories[subjectName].replace('.git',
                ...
                if flag0 or flag1 or flag2:
                    smell = True
                    counts['LongMessageChain'][3] += 1
                else:
                    smell = False
                LongMessageChain.writerow([subjectName,
                                           item[1],
                                           item[1].replace(
                                               '/Users/joaocorreia/PycharmProjects/Pysmell/pysmell/subjects/' + subjectName,
                                               repositories[subjectName].replace('.git', '/blame/master')) + '#L' + str(item[2]),
