...
for smellname in smells.keys():
    ...
    for metricname in smells[smellname].keys():
        ...
        metric.write(
            "%s:  count %d, max %d, min %d, mean %s, plot-box outlier %d-%d, statistic very-high %s, 80th percentile "
            "%s\n\n" \
            % (metricname,
               len(metricdata) if len(metricdata) > 0 else 0,
               max(metricdata) if len(metricdata) > 0 else 0,
               min(metricdata) if len(metricdata) > 0 else 0,
               np.mean(metricdata) if len(metricdata) > 0 else 0,
